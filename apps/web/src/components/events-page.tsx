  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (editingId) {
        ...
        await createEvent({ ...formState, organizerId: user.id });
      }
  ...
  return (
    <div className="space-y-6">
      ...
      {user ? (
        ...
        <Card className="border border-amber-200 bg-white/90">
          <CardHeader>
            <CardTitle>Host a campus event</CardTitle>
            <CardDescription>
              Sign in to create events, manage RSVPs, and edit your listings.
            </CardDescription>
          </CardHeader>
          <CardFooter>
            <Button asChild>
              <a href="/login">Sign in to create events</a>
            </Button>
          </CardFooter>
        </Card>
      )}
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          {eventsByDate.map((event) => (
            <EventCard
              key={event.id}
              event={event}
              isOwner={!!user && event.organizerId === user.id}
